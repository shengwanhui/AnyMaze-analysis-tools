...
def getSubColsByFilter(df, msIDs, filterColName, filterColContent, targetColName):
    subCols = []
    df_filtered = df.where(df[filterColName] == filterColContent)
    df_filtered.set_index('Animal', inplace = True)
    for ms in msIDs:
        subCols.append(df_filtered[targetColName][ms])
    return subCols
